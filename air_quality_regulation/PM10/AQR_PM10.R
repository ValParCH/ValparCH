...
lai<-LAI_mean
forest<-DLT_25_18 #1= deciduous, 2= coniferous
lulc<-raster("C:/Users/kuelling/Documents/VALPAR/DATA/UNIL_data/lulc/LULC_92-95_25.tif")
...
m <- c(0, NA, 1, NA, 2, 0)
c_mat <- matrix(m, ncol=2, byrow=TRUE)
coniferous <- reclassify(forest, c_mat)

#ii. broadleaf forest

m <- c(0, NA, 1, 0, 2, NA)
b_mat <- matrix(m, ncol=2, byrow=TRUE)
broadleaf <- reclassify(forest, b_mat)
...
writeRaster(AQR_index, paste(output,"AQR_CH_25_2015.tif",sep="/"))
writeRaster(AQR, paste(output,"AQR_CH_25_2015_value.tif",sep="/"))
